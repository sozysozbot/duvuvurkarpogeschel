...
void PrivilegedCursoredTextBox::SetTimer(int timer_id, double timer_sec)
{
...
void PrivilegedCursoredTextBox::DrawTextCursor(bool visible) {
  const auto color = visible ? ToColor(0) : ToColor(0xffffff);
  const auto pos = Vector2D<int>{4 + 8*this->cursor_index, 5};
  FillRectangle(*this->text_window->InnerWriter(), pos, {7, 15}, color);
}

void PrivilegedCursoredTextBox::ManageCursor(std::optional<Message> msg) {
  __asm__("cli");
